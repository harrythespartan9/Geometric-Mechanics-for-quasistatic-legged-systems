function [netCurve, CurveTraj, Q_dot, Q_ddot] = extractPathCurvFromQ2(Q, Qdot, ddpsi, A, Adot, params)
...
%constraint and local connection vector fields, and some shape parameters of the SE(2) system.
    ...
    aa      = params{1}; 
    ll      = params{2};
    t       = Q{1};                             t_dot       = ones(size(t));    delt = mean(diff(t));
    x       = Q{2};                             x_dot       = Qdot(1, :);
    y       = Q{3};                             y_dot       = Qdot(2, :);
    theta   = Q{4};                             theta_dot   = Qdot(3, :);      
    ai      = Q{5};                             ai_dot      = Qdot(4, :);
    aj      = Q{6};                             aj_dot      = Qdot(5, :);
    g_dot   = [x_dot; y_dot; theta_dot]; 
    z       = [x(end); y(end); theta(end)]; 
    ...
    CurveTraj = ( vecnorm(Cxy*g_dot, 2) * delt )./(  2*sin( g_dot(3,:) * delt/2 )  );
    netCurve = norm(Cxy*z, 2)/( 2*sin(z(3)/2) );
...
%%
% CurveTraj = (g_dot(1, :).*g_ddot(2, :) - g_dot(2, :).*g_ddot(1, :)) ./ vecnorm(Cxy*g_dot, 2, 1); % old curvature formulation-- incorrect
