function fullSurf = fullStratSurfcompute(stance_i, stance_j, fullStratij, colorRelated)
...
%   computed for each SE(2) direction and provided as the output through a cell array.
    ...
    dnum = size(stance_i, 2); % this will be used to create a square matrix representation of the surface
    ...
    for idx = 1:dnum
        temp_x = interp1([1, dnum], [stance_i(1, idx), stance_j(1, idx)], 1:dnum); temp_x = temp_x(:);
        temp_y = interp1([1, dnum], [stance_i(2, idx), stance_j(2, idx)], 1:dnum); temp_y = temp_y(:); % compute
    ...
    for idxPos = 1:size(fullSurf, 1)
        fullPanelNow = fullStratij(idxPos, :)'; % the current panel
        switch idxPos
            case 3 % if theta panel
                colQ = ( fullPanelNow - colLim.Th(1) )/diff(colLim.Th); % where the current panel slice is in the 1D range
            otherwise % if NOT theta panel
                colQ = ( fullPanelNow - colLim.XY(1) )/diff(colLim.XY);
        end
