...
function kin = qlevel2noslip_clari(kin, plot_info, cs_idx)
...
% unpack user inputs ------------------------------------------------------
cs = kin.submanifolds.list;
fields = kin.submanifolds.fields; configs = kin.submanifolds.configs;
ank = kin.params.ank; ab_ank = kin.params.ab_ank; 
aa = kin.params.aa; ll = kin.params.ll; bl = kin.params.bl; ab0 = kin.params.ab0;
...
% define a 2x2 anticlockwise rotation operation
...
ijm = find(tempX>-ank & tempX<ank, 1); 
ijM = find(tempX>-ank & tempX<ank, 1, 'last');
ii = randi([ijm ijM]); jj = randi([ijm ijM]); % get a random shape over the restricted
                                  % range of [-pi/2,pi/2]
plot_info.i = ii; plot_info.j = jj;

%%%%%%%%%%%%%%%%%%%%%%%%
% uncontrollable shape space discretization
ab = linspace(ab0-ab_ank,ab0+ab_ank,dnum);
...
if isa(bl, 'sym')
    lb = double(subs((bl/perct)^2, l, ll));
else
    lb = (bl/perct)^2;
end
kinfo = [];
kinfo.csi = csi; kinfo.csj = csj;
kinfo.(['h_b__' num2str(kinfo.csi) 'b']) = kin.sym.(['h_b__' num2str(kinfo.csi) 'b']);
kinfo.(['h_b__' num2str(kinfo.csj) 'b']) = kin.sym.(['h_b__' num2str(kinfo.csj) 'b']);
[plot_info.iV, plot_info.ksq_lb] = avoidcontractsingularity(kinfo, ksq, ksq_sweep, aa, ll, lb);
