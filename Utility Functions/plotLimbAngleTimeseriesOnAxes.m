function plotLimbAngleTimeseriesOnAxes(ax, tau, beta, limbAngle, ...
                        stanceColor, swingColor, scatterFlag, lineStyle)
...
%   and limb angle timeseries vector.
    % set the scatter flag default to true if not provided
    if nargin < 8
        lineStyle = '-';
    end
    if nargin < 7
    ...
    for i = 1:numel(switchIdx)
        ...
        if scatterFlag
            scatter(ax, tau(startIdx), limbAngle(startIdx), 100, 'o',...
                "filled", "MarkerFaceColor", scatterfaceStateColor, ...
