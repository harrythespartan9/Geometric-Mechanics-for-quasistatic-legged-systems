function matOut = condition_contour2(temp, iter_old)
%CONDITION_CONTOUR conditions an input contour matrix
%   Sometimes the contour matrices generated for a single level splits it 
%   up over sub-arrays-- the goal is to number each contour result.
    
    % Initialize the return array, a counter, and a point count container (based on an input or fresh start)
    matOut = []; count = size(temp, 2); N = 0;
    switch nargin
        case 2
            iter = iter_old;
        case 1
    ...
    while count > 0
        ...
        matOut = [ matOut, [iter*ones(1, temp(2,1)); temp( :, 2:temp(2,1)+1 )] ];
